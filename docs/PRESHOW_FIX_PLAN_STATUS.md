...
- Pre-flight: the 11-row preflight panel is real and wired (Go Live + Settings) and Outrangutan health IS visible from Live including cross-machine and the ARMED verdict. Missing: OBS, talkback, and deck status appear only inside KeyWi Bird, and there is no always-available readout outside Live. Build: add OBS/talkback/deck rows to the preflight and surface a compact systems chip outside Live.
