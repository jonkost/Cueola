...
5. Command single-slot race: `outrangutan.command` is one field with dedupe + 30s staleness; a panic can be overwritten by a following write before the remote snapshot lands. Consider a command queue or at least panic priority. BUILT 2026-08-29 (panic priority, proto 3): PANIC now also rides its own `outrangutan.panic` field (nothing else writes it, so it cannot be overwritten; same primed-baseline + id-dedupe rules as `outrangutan.gain`, acked through `outrangutan.cmdAck`), a panic purges every in-flight redelivery on the sender (a surviving pad retry could rewrite the slot and restart audio after the kill), and a pending panic is never superseded by a later command. Old Airs still get the legacy slot write + proto-2 retries; the Air stamps `live.proto = 3`. Note the "30s staleness" wording above predates the primed-baseline reader; the staleness check no longer exists. Deploy: reload Outrangutan on the Air to get the panic lane. The sender changes apply regardless of the Air's build but are safe against an un-reloaded Air: proto-2 Airs ignore the extra `outrangutan.panic` field, delivery still rides the legacy slot with proto-2 retries, and a panic now also purges pending pad retries on the sender (deliberate: nothing may restart audio after a kill). Every rundown-side window must also reload so it picks up the cross-window observer purge.
