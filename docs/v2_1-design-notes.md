...
## D12. Live reliability & control-room UX (owner directive 2026-07-20, from the TH2607/P2607 show days)
*(Renumbered from D11 on merge 2026-07-20: D11 was already the live show CONTROL system note from the Jul 17 intake; this is the live RELIABILITY note from Jul 20. Both are never-cut.)*
...
**D12.1 Connection truth + main-bar status.** One shared link-state model (module or common
...
**D12.2 Prompter single-authority session.** Root cause of the Jul 20 split-brain: both sides mint
...
**D12.3 Show-caller certainty.** Jul 20: joining by code lands role `student`
...
**D12.4 Playback first-GO arming.** Symptom (both show days): first GO after entering live did not
...
playout link (D12.1 model) and a preflight row that proves first-fire (fire-and-measure into a
muted sink or readiness assertion, not a heuristic).

**D12.5 Talent overlay discipline + acknowledged toggles.** Owner: overlays must never stop the
...
**D12.6 Questions lane (YouTube chat → talent).** Deliberately dumb: a paste box on the Script Op
...
D12.5). Rides the existing question-flag channel; the broken push-paste path is root-caused and
...
**D12.7 Slim the runtime.** Measure, then defer, then gate. (1) Record budgets on show-class
