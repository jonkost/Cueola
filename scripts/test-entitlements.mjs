...
ok('web (gating off) is full + Outrangutan basic for any account', () => {
  ...
  assert.equal(c.outrangutan, 'basic');
  assert.equal(c.outrangutanUpload, 'limited');
  assert.equal(c.outrangutanPlayout, 'simple');
});

ok('mac (gating off) unlocks full Outrangutan engine', () => {
  assert.equal(E.resolveCapabilities(free, 'mac').outrangutan, 'full');
});

ok('iPad/iPhone NEVER expose Outrangutan (hard rule), even forced full + paid', () => {
  for (const p of ['ipad', 'iphone']) {
    const c = E.resolveCapabilities(paid, p, { gatingEnabled: true });
    assert.equal(c.outrangutan, 'none', p + ' outrangutan');
    assert.equal(c.outrangutanUpload, 'none');
...
ok('gating ON: free web loses Outrangutan, paid web keeps it', () => {
  assert.equal(E.resolveCapabilities(free, 'web', { gatingEnabled: true }).outrangutan, 'none');
  assert.equal(E.resolveCapabilities(paid, 'web', { gatingEnabled: true }).outrangutan, 'basic');
});
...
ok('can() reports availability (none/false → unavailable)', () => {
  ...
  assert.equal(E.can(c, 'outrangutan'), true);
  assert.equal(E.can(E.resolveCapabilities(free, 'ipad'), 'outrangutan'), false);
});
